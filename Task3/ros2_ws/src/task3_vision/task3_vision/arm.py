...
真机安全原则：发送失败、反馈丢失或等待超时都会返回 False，由上层立即停止
当前物体的后续下降/夹取动作，避免机械臂未到位却继续执行。
"""
...
class RealArm:
    ...
    def move(self, arm_deg, label=""):
        ...
        if last is not None:
            ...
            self._warn(f"{label} 等待超时且读不到关节角")
        return False
    ...
    def gripper(self, value, label=""):
        ...
        try:
            ...
            self._warn(f"set_gripper_value 异常: {error}")
            return False
    ...
    def open_gripper(self):
        return self.gripper(
            int(self.config["robot"].get("gripper_open_value", 100)), "张开")

    def close_gripper(self):
        """往死里夹：直接给最紧的位置 + 最大速度。"""
        return self.gripper(
            int(self.config["robot"].get("gripper_close_value", 0)), "夹紧")
